...
@Component({
  selector: 'app-o-cloud-management',
  templateUrl: './o-cloud-management.component.html',
  styleUrls: ['./o-cloud-management.component.scss']
})
export class OCloudManagementComponent implements OnInit {
  ...
  constructor(
    ...
    this.levelMap.set('Deploy Finished', 9);
  }
  ...
  ocloudListDeal() {
    ...
    // refresh
    this.refreshTimeout = setTimeout(() => this.getOcloudList(), this.refreshTime * 1000);
  }
  ...
  create() {
    ...
    if (this.commonService.isLocal) {
      ...
      const url = `${this.commonService.restPath}/createOcloud`;
      const body: any = {
        name: this.createForm.controls['name'].value,
  ...
  delete() {
    if (this.commonService.isLocal) {
      ...
      const url = `${this.commonService.restPath}/deleteOcloud`;
      const body = {
        id: this.selectOcloud.id
      }
      //this.http.post(url, JSON.stringify(body)).subscribe(
	this.http.post(url, this.selectOcloud.id).subscribe(  
        res => {
  ...
  viewPage(oCloudList: OCloudList) {
    this.router.navigate(['/main/o-cloud-mgr/info', oCloudList.id]);
  }
